...
import { RegionList } from "../../config/RegionList";

export class RegionsAntTableSmart extends AntTableSmart<Region> {
  collectionClass = RegionsCollection;

  getBody(): QueryBodyType<Region> {
    return RegionList.getRequestBody();
  }

  getColumns(): ColumnsType<Region> {
    const list = this.container.get(RegionList);
    list.build();

    return [...list.rest(), this.getActionsColumn()];
  }

  getActionsColumn(): ColumnType<Region> {
    return {
  ...
  getActionItems() {
    ...
    if (features.view) {
      actions.push({
        ...
        icon: <EyeOutlined />,
        action: (model) => {
          this.router.go(Routes.REGIONS_VIEW, {
    ...
    if (features.edit) {
      actions.push({
        ...
        icon: <EditOutlined />,
        action: (model) => {
          this.router.go(Routes.REGIONS_EDIT, {
            params: { id: model._id.toString() },
          });
        },
      });

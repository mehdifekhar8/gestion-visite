...
import { DoctorEditForm } from "../../config/DoctorEditForm";
...
export function DoctorsEdit(props: { id: string }) {
  const UIComponents = useUIComponents();
  const t = useTranslate();
  const form = use(DoctorEditForm, { transient: true });
  ...
  return (
    <UIComponents.AdminLayout>
      <Ant.PageHeader
        title={t("management.doctors.edit.header")}
        ...
        extra={
          features.view
            ? [
                <Link
                  key="view"
                  to={router.path(Routes.DOCTORS_VIEW, {
                    params: { id: props.id },
                  })}
                >

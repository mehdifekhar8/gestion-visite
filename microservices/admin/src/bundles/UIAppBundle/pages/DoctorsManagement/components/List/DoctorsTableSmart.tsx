...
import { DoctorList } from "../../config/DoctorList";

export class DoctorsAntTableSmart extends AntTableSmart<Doctor> {
  collectionClass = DoctorsCollection;

  getBody(): QueryBodyType<Doctor> {
    return DoctorList.getRequestBody();
  }

  getColumns(): ColumnsType<Doctor> {
    const list = this.container.get(DoctorList);
    list.build();

    return [...list.rest(), this.getActionsColumn()];
  }

  getActionsColumn(): ColumnType<Doctor> {
    return {
  ...
  getActionItems() {
    ...
    if (features.view) {
      actions.push({
        ...
        icon: <EyeOutlined />,
        action: (model) => {
          this.router.go(Routes.DOCTORS_VIEW, {
    ...
    if (features.edit) {
      actions.push({
        ...
        icon: <EditOutlined />,
        action: (model) => {
          this.router.go(Routes.DOCTORS_EDIT, {
            params: { id: model._id.toString() },
          });
        },
      });

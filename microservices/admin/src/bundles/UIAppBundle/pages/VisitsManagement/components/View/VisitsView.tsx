...
import { VisitViewer } from "../../config/VisitViewer";
import { features } from "../../config/features";

export function VisitsView(props: { id: string }) {
  ...
  if (isLoading) {
    ...
    if (error || document === null) {
      ...
      content = <VisitsViewComponent document={document} />;
    }
  }
  ...
  return (
    <UIComponents.AdminLayout>
      <Ant.PageHeader
        title={t("management.visits.view.header")}
...
export function VisitsViewComponent(props: { document: Partial<Visit> }) {
  const document = props.document;

  const viewer = use(VisitViewer, { transient: true });
...
export function getHeaderActions(
  ...
  if (features.edit) {
    actions.push(
      <Link
        key="edit"
        to={router.path(Routes.VISITS_EDIT, {
          params: { id },
        })}
      >
      ...
      <Ant.Popconfirm
        ...
        onConfirm={() => {
          collection.deleteOne(id).then(() => {
            router.go(Routes.VISITS_LIST);
            Ant.notification.success({
              message: "Success",
              description: "You have deleted the Visit",
            });
          });
        }}
      >
  ...
  const viewRoutePath = router.path(Routes.VISITS_VIEW, {
    params: { id },
  });
